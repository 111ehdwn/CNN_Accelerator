...
사용자는 현재 macbook 환경에서 작업하고, parallel desktop 내 vivado에 코드를 그대로 복사 붙여넣기하고, ip 설정은 생성 후 캡처본을 디렉터리 내에 넣는 방식으로
진행 중임. 따라서 코드 변경 후 어떤 코드를 변경했고 복사 붙여넣기 하면 되는지 알릴 것.
