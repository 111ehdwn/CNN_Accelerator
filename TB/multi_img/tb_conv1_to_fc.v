...
module tb_conv1_to_fc;
    ...
    // conv1_weight_bram (외부 BMG, conv1_engine 은 w_bram_* external port 방식)
    reg          w1_ena   = 1'b0;
    reg          w1_wea   = 1'b0;
    reg  [5:0]   w1_addra = 6'd0;
    reg  [31:0]  w1_dina  = 32'd0;
    wire [5:0]   w1_addrb;   // conv1_engine 이 drive
    wire         w1_enb;     // conv1_engine 이 drive
    wire [31:0]  w1_doutb;   // BMG → conv1_engine
    ...
    // read port (fc_engine ← poolfc, L=1)
    always @(posedge clk) begin
        if (poolfc_re)
    ...
    //==========================================================================
    // DUT 1: Conv1
    //   weight BRAM 은 외부 인스턴스 (w1_bmg) — w_bram_* external port 방식
    //==========================================================================
    conv1_engine conv1 (
        .clk          (clk),
        .rst          (rst),
        ...
        .w_bram_addr  (w1_addrb),             // 기존 TB 포트명과 동일
        .w_bram_en    (w1_enb),
    ...
    //==========================================================================
    task init_weight1;
        // conv1_weight_bram Port A (외부 BMG) 에 직접 write
        integer wi;
        begin
    ...
    initial begin : main_process
        ...
        // Conv2 start pulse (weight load 1회)
        ...
        $display("  total cycles (conv2_start → last class_valid) : %0d",
    ...
    //   ★ maxpool.succ_rdone 는 fc_rdone direct wire (TB pulse 없음)
    //==========================================================================
    integer i_cmp;
    reg [3:0] got_class;
