...
concatenated big-file 만 dump (multi_img TB 의 $readmemh 용):
  ...
  - ../../vitis/test_images.h      : Vitis main.c 용 (test_images[N*784] + test_labels[N]=argmax raw logit)
...
def write_test_images_header(inp_arr, logit, n, path):
    ...
    test_images[n*784] : raster byte (all_input.hex 와 동일 데이터, conv1 입력).
    ...
    L = [
        ...
        "// test_images : N x 784 raster bytes (conv1 입력, all_input.hex 와 동일 데이터)",
        ...
        "#define TEST_IMG_BYTES 784",
        "",
        "static const uint8_t test_images[TEST_N_IMAGES * TEST_IMG_BYTES] = {",
    ]
    for i in range(n):
        b = build_input_bytes(inp_arr, i)
        L.append(f"    /* img {i} (label {int(labels[i])}) */")
        for off in range(0, 784, 16):
            L.append("    " + "".join(f"0x{v:02X}u," for v in b[off:off + 16]))
    L.append("};")
    L.append("")
    ...
    with open(path, "w") as f:
        f.write("\n".join(L) + "\n")
    print(f"  {path}  (N={n}, {n * 784} bytes, labels[:10]={labels[:10].tolist()})")
