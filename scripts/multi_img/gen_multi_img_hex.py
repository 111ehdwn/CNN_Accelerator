...
concatenated big-file 만 dump (multi_img TB 의 $readmemh 용):
  ...
  - ../../vitis/test_images.h      : Vitis main.c 용 (test_images[N*196] uint32 pre-packed + test_labels[N]=argmax raw logit)
...
VITIS_DIR    = os.path.join(HERE, "../../vitis")          # Vitis main.c 용 test_images.h
...
def build_input_words(inp_arr, img_idx):
    """784 raster byte → 196 little-endian uint32 word (input BRAM Port A 32-bit 포맷).

    word k = b[4k] | b[4k+1]<<8 | b[4k+2]<<16 | b[4k+3]<<24.
    bram_input (asymmetric 32/8) 의 little-endian byte order 와 일치 — conv1 Port B 가
    addr 4k+0 에서 byte0(=word[7:0]) 를 read. → PS 가 런타임 패킹 없이 그대로 Xil_Out32.
    (784 = 4 × 196 정확히 나누어떨어져 padding 불필요.)
    """
    b = build_input_bytes(inp_arr, img_idx)            # 784 byte (0..255)
    return [(b[4 * k] | (b[4 * k + 1] << 8) | (b[4 * k + 2] << 16) | (b[4 * k + 3] << 24))
            for k in range(196)]


def write_test_images_header(inp_arr, logit, n, path):
    """vitis/test_images.h 생성 (Vitis main.c 용).

    test_images[n*196] : input BRAM Port A 포맷으로 **미리 패킹된 uint32 word**
      (raster byte 4개를 little-endian 결합; all_input.hex 와 동일 데이터).
      → main.c 가 런타임 byte 패킹 없이 Xil_Out32(p[i]) 로 바로 전송 (PS 전송 병목 완화).
    test_labels[n]     : argmax(raw FC logit) = HW fc_argmax 기대 class.
      ★ raw 24-bit logit 기준 (fc_argmax 가 logit_reg=raw 누적값에 동작). saturate(fc_sat)
        argmax 와 드물게 다를 수 있어 logit 으로 계산. np.argmax = 낮은 index tie-break
        (fc_argmax strict '>' 와 일치). MNIST ground-truth 아님 (repo 에 라벨 파일 없음;
        input.npy 가 MNIST train 순서라 보통 true label 과 일치).
    """
    labels = np.argmax(logit[:n], axis=1)
    L = [
        ...
        "#include <stdint.h>",
        "",
        "// auto-generated by scripts/multi_img/gen_multi_img_hex.py",
        "// test_images : N x 196 pre-packed uint32 words (input BRAM Port A 32-bit 포맷).",
        "//   word k = b[4k] | b[4k+1]<<8 | b[4k+2]<<16 | b[4k+3]<<24  (little-endian, all_input.hex 와 동일 데이터).",
        "//   → PS(main.c)는 패킹 없이 Xil_Out32(test_images[img*196 + i]) 로 직접 전송.",
        "// test_labels : argmax(raw FC golden logit) = HW fc_argmax 기대 class (HW==golden 검증용).",
        "//   (MNIST ground-truth label 아님 — repo 에 라벨 파일 없음.)",
        "",
        f"#define TEST_N_IMAGES {n}",
        "#define TEST_IMG_WORDS 196   /* 784 byte / 4 = input BRAM Port A words per image */",
        "",
        "static const uint32_t test_images[TEST_N_IMAGES * TEST_IMG_WORDS] = {",
    ]
    for i in range(n):
        words = build_input_words(inp_arr, i)
        L.append(f"    /* img {i} (label {int(labels[i])}) */")
        for off in range(0, 196, 8):
            L.append("    " + "".join(f"0x{v:08X}u," for v in words[off:off + 8]))
    L.append("};")
    L.append("")
    L.append("static const uint8_t test_labels[TEST_N_IMAGES] = {")
    for off in range(0, n, 20):
        L.append("    " + "".join(f"{int(v)}," for v in labels[off:off + 20]))
    L.append("};")
    L.append("")
    L.append("#endif /* TEST_IMAGES_H */")

    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w") as f:
        f.write("\n".join(L) + "\n")
    print(f"  {path}  (N={n}, {n * 196} words = {n * 784} bytes, labels[:10]={labels[:10].tolist()})")
...
# ---------------------------------------------------------------------------
# Vitis main.c 용 test image header (같은 forward 데이터에서 생성 → sim 과 정합)
# ---------------------------------------------------------------------------
print(f"\n[Write] Vitis test image header → {VITIS_DIR}")
write_test_images_header(images, fc_logit, N_IMAGES,
                         os.path.join(VITIS_DIR, "test_images.h"))
