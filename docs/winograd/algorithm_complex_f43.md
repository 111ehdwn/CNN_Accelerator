...
> ## ⚠️ 행렬 정정 (2026-06-04, golden 검증으로 발견 — RTL 은 §9.1 정정판 사용)
> 아래 §4.2/§4.3 의 **Bᵀ, A'ᵀ 는 다항식 곱(product) 구성** (Vandermonde Bᵀ + Lagrange A'ᵀ)
> 으로, CNN correlation `y_n=Σ_k g_k·d_{n+k}` 와 일치하지 않는다(golden 결과 direct 대비 45~54%만 일치).
> CNN conv 은 **convolution(transpose) 구성**이 필요. 수치 유도+검증(`scripts/golden_sim/1_complex_winograd_f(4,3).py`,
> err 5e-16, **전체 10000장 bit-exact 100%**)으로 정정한 행렬은 **§9.1** 참조. 요지:
> - **G** (필터, {0,±1,±i}) — 동일 ✓
> - **Bᵀ** (입력) = `{0,±1,±i,±4}` ← ¼ 스케일이 **입력 Bᵀ** 에 (문서는 출력 A'ᵀ 에 둠)
> - **Aᵀ** (출력) = `{0,±1,±i}` 평가 Vandermonde ← ×4 **없음** (문서와 A/B 역할이 뒤바뀜)
>
> 결론(Gaussian integer / 곱셈 46 / 출력 >>14 / U=GgGᵀ 사전계산 / bit-exact)은 그대로 유효 —
> 입력·출력 변환 *행렬값* 만 정정. §4.3 의 Lagrange 유도가 product 를 계산한 것이 근본 원인.
...
U = G · g · G^T  (정수, 사전 계산) — G 스케일 안 함. 1/16 은 출력 >>4 (★Bᵀ=4·(true) 양쪽 → V=16·V_true → layer >>10 과 합쳐 >>14)

★정정판 (golden 검증, 전체 10000장 bit-exact). ¼ 스케일은 **입력 Bᵀ** 에:

Bᵀ (입력 변환, 6×6, ×4) — 원소 {0, ±1, ±i, ±4}:
[
   4    0    0    0   -4    0
   0    1    1    1    1    0
   0   -1    1   -1    1    0
...
Aᵀ (출력 변환, 4×6) — 평가 Vandermonde, 원소 {0, ±1, ±i} (×4 없음):
[
   1    1    1    1    1    0
   0    1   -1    i   -i    0
   0    1    1   -1   -1    0
...
(우측 곱은 transpose: V = Bᵀ·d·B (B=Bᵀᵀ), Y16 = Aᵀ·M·A (A=Aᵀᵀ). 곱셈기 0개, 시프트+부호+i-swap+×4.)
````
...
````
U   = G · g · Gᵀ          (6×6 complex 정수, 사전 계산; G 스케일 없음)
V   = Bᵀ · d · B          (6×6 complex 정수; ★정정판 Bᵀ=×4 → V=16·V_true)
M   = U ⊙ V               (6×6 complex, element-wise, Σ_IC)
Y16 = Aᵀ · M · A          (4×4 real, = 16·Y_true)   →  out = sat(Y16 >> 14)
※ 행렬은 §9.1 정정판 사용 (golden bit-exact 검증). §4 inline 유도는 product 구성이라 오류.
````
